...
export default function StatsClient({ initialData }: { initialData: StatsData }) {
  ...
  const avgPerYear = data.rollsPerYear.length > 0
  ...
  const totalScanned = scannedStatuses.reduce((s, st) => s + (statusMap[st] ?? 0), 0);
  const pctScanned = totalRolls > 0 ? Math.round((totalScanned / totalRolls) * 100) : 0;
          ...
          <div className="space-y-2.5">
            {data.rollsPerYear.map((r) => (
              <div key={r.year} className="flex items-center gap-3">
                ...
                <span className="text-sm font-semibold w-6 text-right tabular-nums">{r.count}</span>
              </div>
            ))}
          </div>
        </section>
            ...
            {STATUS_ORDER.filter((s) => statusMap[s]).map((status) => {
              ...
              return (
                <div key={status} className="flex items-center gap-3">
                  <span className={`text-xs font-semibold w-20 shrink-0 ${STATUS_LABEL_COLOR[status]}`}>
                    {status}
                  </span>
                  ...
                  <span className="text-sm font-semibold w-6 text-right tabular-nums">{count}</span>
                </div>
              );
            })}
          </div>
...
function HeroStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-xl px-3 py-4 text-center border border-zinc-100 dark:border-transparent">
      ...
      <div className="text-[11px] text-zinc-500 mt-1 uppercase tracking-wide">{label}</div>
    </div>
  );
}

function RankedList({ items, max }: { items: { label: string; count: number }[]; max: number }) {
  return (
    <div className="space-y-2.5">
      {items.map((r, i) => (
        <div key={r.label} className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-semibold text-zinc-500 shrink-0 tabular-nums ml-auto">{r.count}</span>
            </div>
