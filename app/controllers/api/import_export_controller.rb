module Api
  class ImportExportController < BaseController
    # POST /api/import
    def import
      cameras_data = Array(params[:cameras]).filter_map { |c| c.respond_to?(:to_unsafe_h) ? c.to_unsafe_h.with_indifferent_access : (c.respond_to?(:with_indifferent_access) ? c.with_indifferent_access : nil) }
      films_data   = Array(params[:films]).filter_map   { |f| f.respond_to?(:to_unsafe_h) ? f.to_unsafe_h.with_indifferent_access : (f.respond_to?(:with_indifferent_access) ? f.with_indifferent_access : nil) }
      rolls_data   = Array(params[:rolls]).filter_map   { |r| r.respond_to?(:to_unsafe_h) ? r.to_unsafe_h.with_indifferent_access : (r.respond_to?(:with_indifferent_access) ? r.with_indifferent_access : nil) }
